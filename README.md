...
단순히 만들어진 라이브러리를 사용하는 것을 넘어, **"모델 내부의 핵심 로직과 수학적 원리를 이해해야만 실무의 엣지 케이스(Edge Case)를 해결할 수 있다"** 고 믿습니다. 논문을 직접 구현하며 다진 '기초 체력' 위에, **밑바닥(From Scratch) → 전이학습/파인튜닝 → SOTA 모델 최적화 및 서빙(Triton/TensorRT)** 까지 이어지는 엔드투엔드 AI 파이프라인을 설계하고 있습니다.
...
* **⭐ [`Fashion-King-Virtual-Fitting`](https://github.com/AD-Styles/Fashion-King-Virtual-Fitting)** — **360° 가상 피팅과 AI 색상 추천을 결합한 웹 패션 시뮬레이터 '내일은 패션왕'.** 체형 슬라이더로 만든 3D 마네킹에 옷을 입혀 360°로 돌려보고, **얼굴 사진 한 장으로 퍼스널 컬러를 진단해 어울리는 코디 색까지** 추천하는 엔드투엔드 시스템. 프론트는 **Next.js 14 + Three.js / React Three Fiber**로 체형 7-파라미터 마네킹·의류 GLB 스키닝·OrbitControls 360° 회전을 실시간 렌더링하고, 옷 사진→가상 착용은 **IDM-VTON** 합성 파이프라인으로 연동. 핵심 담당 모듈인 **2D 색상 추천 시스템**은 **ResNet18 멀티태스크 CNN**이 얼굴에서 웜쿨·명도·선명도 **3축을 회귀**해 **사계절 12타입**으로 분류 — **시즌 정확도 90.4% / 타입 79.5% (축 MAE 0.14~0.27)**. 라벨은 FairFace 코퍼스에 **SegFormer 피부·모발 세그멘테이션 + 2-pass 부트스트랩**으로 인구통계 편향(피부 황색도·명도)을 코퍼스 통계로 보정해 자동 생성하고, *색이 곧 신호*이므로 **ColorJitter 색 증강을 배제**(기하 증강만)해 라벨 오염을 차단. 배포는 **torch 없이 ONNX Runtime**만으로 추론하며 **mediapipe 1.25× 얼굴 크롭**으로 학습-추론 분포를 일치시켜 정확도 저하를 방지. 추천은 **CIELab ΔE76 지각 거리**로 시즌 팔레트와 의류 색을 비교·정렬(ΔE<12 잘어울림 / <28 무난), **FastAPI `/personal-color`** 엔드포인트와 React 결과 패널까지 연동 완성.
